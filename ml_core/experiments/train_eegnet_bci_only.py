"""Experiment 1: train EEGNet on BCI IV-2a only, without PhysioNet pretraining.

Usage:
    python -m ml_core.experiments.train_eegnet_bci_only \
        --delta-path delta_lake/epochs_mi_v1_ch5_sr128_bp8_30 \
        --filter-version bp_8_30_v1 \
        --out-dir artifacts/checkpoints/eegnet_bci_only \
        --use-ea
"""
...
def main() -> None:
    """Train EEGNet from scratch on BCI IV-2a and evaluate held-out subjects."""
    configure_logging()
    parser = argparse.ArgumentParser(description="EEGNet BCI-only baseline")
    ...
    cb = build_callback(
        args,
        run_name="eegnet_bci_only",
        tags={"phase": "bci_only", "model": "EEGNet", "dataset": "bci_iv_2a"},
    )
